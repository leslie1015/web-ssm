...
public class AutoGeneratorTool {
  ...
  private static AutoGenerator setConfig() {
    ...
    gc.setOutputDir(getProjectPath() + "/src/main/java"); // 生成文件的输出目录【默认 D 盘根目录】
    gc.setAuthor("wangyong"); // [开发者]
    gc.setFileOverride(false); // 开启文件覆盖
      ...
      AutoGenerator mpg = setConfig();
        ...
        focList.add(new FileOutConfig(vmTemplatePath) {
            ...
            public String outputFile(TableInfo tableInfo) {
                // 自定义mapper文件名称
                return getProjectPath() + "/src/main/resources/mapping/" + tableInfo.getEntityName() + "Mapper" + StringPool.DOT_XML;
            }
